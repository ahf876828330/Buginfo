...
class Seq2Seq(nn.Module):
    ...
    def __init__(
        ...
        self.infotrg_vocab_size = infotrg_vocab_size
        ...
        self.pad_token_infotrg = pad_token_infotrg
        ...
        self.infotrg_embedding = nn.Embedding(
            infotrg_vocab_size,
            trg_emb_dim,
            self.pad_token_infotrg
        )
        self.encoder = nn.LSTM(
            src_emb_dim,
            self.src_hidden_dim,
        ...
        self.decoder2infovocab = nn.Linear(trg_hidden_dim,infotrg_vocab_size).cuda()
        self.init_weights()
    ...
    def init_weights(self):
        ...
        self.infotrg_embedding.weight.data.uniform(-initrange, initrange)
        self.encoder2decoder.bias.data.fill_(0)
        self.decoder2vocab.bias.data.fill_(0)
        self.decoder2infovocab.bias.data.fill_(0)
    ...
    def forward(self, input_src, input_trg,input_infotrg,ctx_mask=None, trg_mask=None):
        """Propogate input through the network."""
        src_emb = self.src_embedding(input_src)
        trg_emb = self.trg_embedding(input_trg)
        infotrg_emb = self.infotrg_embedding(input_infotrg)
        ...
        trg_h, (_, _) = self.code_decoder(
            trg_emb,
            (
                decoder_init_state.view(
                    self.code_decoder.num_layers,
        ...
        infotrg_h, (_, _) = self.info_decoder(
            infotrg_emb,
            (
                decoder_init_state.view(
                    self.info_decoder.num_layers,
                    decoder_init_state.size(0),
                    decoder_init_state.size(1)
                ),
                c_t.view(
        ...
        infotrg_h_reshape = infotrg_h.contiguous().view(
            infotrg_h.size(0) * infotrg_h.size(1),
            infotrg_h.size(2)
        )
        code_decoder_logit = self.decoder2vocab(trg_h_reshape)
        info_decoder_logit = self.decoder2infovocab(infotrg_h_reshape)
        code_decoder_logit = code_decoder_logit.view(
            trg_h.size(0),
            trg_h.size(1),
            code_decoder_logit.size(1)
        )
        info_decoder_logit = info_decoder_logit.view(
            infotrg_h.size(0),
            infotrg_h.size(1),
            info_decoder_logit.size(1)
        )
        return code_decoder_logit,info_decoder_logit
...
class Seq2SeqAttention(nn.Module):
    ...
    def __init__(
        ...
        self.pad_token_src = pad_token_src
        self.pad_token_trg = pad_token_trg
        self.pad_token_infotrg = pad_token_infoarg
        ...
        self.code_decoder = LSTMAttentionDot(
            trg_emb_dim,
            trg_hidden_dim,
            batch_first=True
        )

        self.info_decoder = LSTMAttentionDot(
            trg_emb_dim,
            trg_hidden_dim,
            batch_first=True
        )
        self.encoder2code_decoder = nn.Linear(
            self.src_hidden_dim * self.num_directions,
            trg_hidden_dim
        )
        self.encoder2info_decoder = nn.Linear(
            self.src_hidden_dim * self.num_directions,
            trg_hidden_dim
        )
        self.code_decoder2vocab = nn.Linear(trg_hidden_dim, trg_vocab_size)
        self.info_decoder2vocab = nn.Linear(trg_hidden_dim, infotrg_vocab_size)
        self.init_weights()
    ...
    def init_weights(self):
        ...
        self.src_embedding.weight.data.uniform_(-initrange, initrange)
        self.trg_embedding.weight.data.uniform_(-initrange, initrange)
        self.infotrg_embedding.weight.data.uniform_(-initrange, initrange)
        self.encoder2code_decoder.bias.data.fill_(0)
        self.encoder2info_decoder.bias.data.fill_(0)
        self.code_decoder2vocab.bias.data.fill_(0)
        self.info_decoder2vocab.bias.data.fill_(0)
    ...
    def forward(self, input_src, input_trg,input_infotrg, trg_mask=None, ctx_mask=None):
        """Propogate input through the network."""
        src_emb = self.src_embedding(input_src)
        trg_emb = self.trg_embedding(input_trg)
        infotrg_emb = self.infotrg_embedding(input_infotrg)
        ...
        if self.bidirectional:
            ...
            h_t = src_h_t[-1]
            c_t = src_c_t[-1]
        code_decoder_init_state = nn.Tanh()(self.encoder2code_decoder(h_t))
        info_decoder_init_state = nn.Tanh()(self.encoder2info_decoder(h_t))
        ctx = src_h.transpose(0, 1)

        trg_h, (_, _) = self.code_decoder(
            trg_emb,
            (code_decoder_init_state, c_t),
            ctx,
            ctx_mask
        )
        infotrg_h, (_, _) = self.info_decoder(
            infotrg_emb,
            (info_decoder_init_state, c_t),
            ctx,
            ctx_mask
        )
        trg_h_reshape = trg_h.contiguous().view(
        ...
        code_decoder_logit = self.code_decoder2vocab(trg_h_reshape)
        info_decoder_logit = self.info_decoder2vocab(infotrg_h_reshape)
        code_decoder_logit = code_decoder_logit.view(
            trg_h.size()[0],
            trg_h.size()[1],
            code_decoder_logit.size()[1]
    ...
    def code_decode(self, logits):
        ...
        return word_probs
    def info_decode(self, logits):
        """Return probability distribution over words."""
        logits_reshape = logits.view(-1, self.infotrg_vocab_size)
        word_probs = F.softmax(logits_reshape)
        word_probs = word_probs.view(
            logits.size()[0], logits.size()[1], logits.size()[2]
        )
