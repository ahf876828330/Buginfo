...
def decode_minibatch(
    ...
    for i in range(config['data']['max_trg_length']):
        ...
        code_probs = model.code_decode(code_decoder_logit)
        info_probs = model.info_decode(info_decoder_logit)
        code_decoder_argmax = code_probs.data.cpu().numpy().argmax(axis=-1)
        info_decoder_argmax = info_probs.data.cpu().numpy().argmax(axis=-1)
        code_next_preds = Variable(
            torch.from_numpy(code_decoder_argmax[:, -1])
        ).cuda()
        info_next_preds = Variable(
            torch.from_numpy(info_decoder_argmax[:, -1])
        ).cuda()
        input_lines_trg = torch.cat(
            (input_lines_trg, code_next_preds.unsqueeze(1)),
            1
        )
        input_lines_infotrg = torch.cat(
            (input_lines_infotrg, info_next_preds.unsqueeze(1)),
            1
        )
    return input_lines_trg,input_lines_infotrg
...
def evaluate_model(
    ...
    trg_test,infotrg_test, config, src_valid=None, trg_valid=None,
    verbose=True, metric='bleu'
):
    """Evaluate model."""
    preds_code,preds_info = [],[]
    code_ground_truths,info_ground_truths = [],[]
    for j in range(0, len(src_test['data']), config['data']['batch_size']):

        # Get source minibatch
        input_lines_src, output_lines_src, _, _ = get_minibatch(
            src_test['data'], src['word2id'], j, config['data']['batch_size'],
            config['data']['max_src_length'], add_start=True, add_end=True
        )

        # Get target minibatch
        input_lines_trg_gold, output_lines_trg_gold, _, _ = (
            get_minibatch(
                trg_test['data'], trg['word2id'], j,
                config['data']['batch_size'], config['data']['max_trg_length'],
                add_start=True, add_end=True
        ...
        input_lines_infotrg_gold, output_lines_infotrg_gold, _, _ = (
            get_minibatch(
                infotrg_test['data'], infotrg['word2id'], j,
                config['data']['batch_size'], config['data']['max_trg_length'],
                add_start=True, add_end=True
            )
        )
        ...
        input_lines_trg = Variable(torch.LongTensor(
            [
                [trg['word2id']['<s>']]
                for _ in range(input_lines_src.size(0))
            ]
        )).cuda()
        input_lines_infotrg = Variable(torch.LongTensor(
            [
                [infotrg['word2id']['<s>']]
                for _ in range(input_lines_src.size(0))
            ]
        ...
        input_lines_trg,input_lines_infotrg = decode_minibatch(
            config, model, input_lines_src,
            input_lines_trg,input_lines_infotrg
        )

        # Copy minibatch outputs to cpu and convert ids to words
        input_lines_trg = input_lines_trg.data.cpu().numpy()
        input_lines_infotrg = input_lines_infotrg.data.cpu().numpy()
        input_lines_trg = [
            [trg['id2word'][x] for x in line]
            for line in input_lines_trg
        ]
        input_lines_infotrg = [
            [infotrg['id2word'][x] for x in line]
            for line in input_lines_infotrg
        ]
        # Do the same for gold sentences
        output_lines_trg_gold = output_lines_trg_gold.data.cpu().numpy()
        output_lines_infotrg_gold = output_lines_infotrg_gold.data.cpu().numpy()
        output_lines_trg_gold = [
            [trg['id2word'][x] for x in line]
            for line in output_lines_trg_gold
        ]
        output_lines_infotrg_gold = [
            [infotrg['id2word'][x] for x in line]
            for line in output_lines_infotrg_gold
        ]
        # Process outputs
        for sentence_pred, sentence_real, sentence_real_src in zip(
            ...
            preds_code.append(['<s>'] + sentence_pred[:index + 1])
            ...
            if verbose:
                print('--------------------------------------')
            code_ground_truths.append(['<s>'] + sentence_real[:index + 1])
        for sentence_pred, sentence_real, sentence_real_src in zip(
            input_lines_infotrg,
            output_lines_infotrg_gold,
            output_lines_src
            ...
            if '</s>' in sentence_pred:
                ...
                index = len(sentence_pred)
            preds_info.append(['<s>'] + sentence_pred[:index + 1])
            ...
            if verbose:
                print('--------------------------------------')
            info_ground_truths.append(['<s>'] + sentence_real[:index + 1])

    return get_bleu(preds_code, code_ground_truths),get_bleu(preds_info,info_ground_truths)
