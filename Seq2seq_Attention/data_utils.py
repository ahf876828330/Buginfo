...
def read_nmt_data(src, config, trg=None,infotrg=None):
    """Read data from files."""
    print('Reading source data ...')
    src_lines = []
    with open(src, 'r') as f:
    ...
    if infotrg is not None:
        print('Reading buginfo data ...')
        infotrg_lines = []
        with open(infotrg, 'r') as f:
            for line in f:
                infotrg_lines.append(line.strip().split())

        print('Constructing buginfo vocabulary ...')
        infotrg_word2id, infotrg_id2word = construct_vocab(
            infotrg_lines, config['data']['n_words_trg']
        )

        infotrg = {'data': infotrg_lines, 'word2id': infotrg_word2id, 'id2word': infotrg_id2word}
    else:
        infotrg = None
    return src, trg, infotrg
